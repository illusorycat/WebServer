...
#define HTTP_WERSERVER_HTML_ROOT	"./html"
...
int readline(char* allbuf, int idx, char* linebuf)//第一个参数是输入参数，第二个参数是起始位置，第三个参数是输入参数
{
...
int http_request(struct ntyevent* ev)
{
	...
	if (strstr(linebuf, "GET"))
	{
		...
		sprintf(ev->resource, "%s/%s", HTTP_WERSERVER_HTML_ROOT, linebuf+sizeof("GET "));
...
int http_response(struct ntyevent* ev)
{
	...
	{//404
		...
		ev->length = sprintf(ev->buffer,
			...
			"Date: Thu, 10 Mar 2022 12:05:52 GMT\r\n"
